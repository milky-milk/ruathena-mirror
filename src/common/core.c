...
const char* get_svn_revision(void)
{
	 ...
	 **/
	if(!(*rA_svn_version))
		snprintf(rA_svn_version, sizeof(rA_svn_version), "Неизвестно");
...
static void display_title(void) {
	...
	ShowMessage(""CL_PASS"          "CL_BT_WHITE"     Команда разработчиков проекта rAthena представляют    "CL_PASS""CL_CLL""CL_NORMAL"\n");
	...
	ShowMessage(""CL_PASS"        "CL_BOLD"                                                              "CL_PASS""CL_CLL""CL_NORMAL"\n");  
	ShowMessage(""CL_PASS"          "CL_GREEN"                   http://rathena.ru/                    "CL_PASS""CL_CLL""CL_NORMAL"\n");
	ShowMessage(""CL_PASS"        "CL_BOLD"                                                              "CL_PASS""CL_CLL""CL_NORMAL"\n"); 

	ShowInfo("SVN Ревизия: '"CL_WHITE"%s"CL_RESET"'.\n", get_svn_revision());
}
...
#ifndef _WIN32
    if (geteuid() == 0) {
		ShowWarning ("Вы запустили rAthena с правами суперпользователя, в этом нет необходимости.\n");
    }
#endif
}
