...
#define BUFVPRINTF(buf,fmt,args)						\
	...
	{/* dynamic buffer */								\
		...
		ShowDebug("showmsg: используется динамический буфер, увеличение размера буфера до %d или больше.\n", buf.l_+1);\
	}													\
//define BUFVPRINTF
...
int _vShowMessage(enum msg_type flag, const char *string, va_list ap)
{
	...
	if (!string || *string == '\0') {
		ShowError("В функцию _vShowMessage() передана пустая строка.\n");
		return 1;
	}
	/**
	...
	if(
		...
		if( (log = fopen(SERVER_TYPE == ATHENA_SERVER_MAP ? "./log/map-msg_log.log" : "./log/unknown.log","a+")) ) {
			...
			fprintf(log,"(%s) [ %s ] : ",
				timestring,
				flag == MSG_WARNING ? "Предупреждение" :
				flag == MSG_ERROR ? "Ошибка" :
				flag == MSG_SQL ? "SQL Ошибка" :
				flag == MSG_DEBUG ? "Отладка" :
				"Неизвестно");
			va_copy(apcopy, ap);
	...
	switch (flag) {
		...
		case MSG_STATUS: //Bright Green (To inform about good things)
			strcat(prefix,CL_GREEN"[Статус]"CL_RESET":");
			break;
		...
		case MSG_INFORMATION: //Bright White (Variable information)
			strcat(prefix,CL_WHITE"[Инфо]"CL_RESET":");
			break;
		case MSG_NOTICE: //Bright White (Less than a warning)
			strcat(prefix,CL_WHITE"[Анонс]"CL_RESET":");
			break;
		case MSG_WARNING: //Bright Yellow
			strcat(prefix,CL_YELLOW"[Предупреждение]"CL_RESET":");
			break;
		case MSG_DEBUG: //Bright Cyan, important stuff!
			strcat(prefix,CL_CYAN"[Отладка]"CL_RESET":");
			break;
		case MSG_ERROR: //Bright Red  (Regular errors)
			strcat(prefix,CL_RED"[Ошибка]"CL_RESET":");
			break;
		case MSG_FATALERROR: //Bright Red (Fatal errors, abort(); if possible)
			strcat(prefix,CL_RED"[Фатальная Ошибка]"CL_RESET":");
			break;
		default:
			ShowError("В функцию _vShowMessage() -> передан недопустимый флаг.\n");
			return 1;
	}
	...
	if(strlen(DEBUGLOGPATH) > 0) {
		...
		if (fp == NULL)	{
			FPRINTF(STDERR, CL_RED"[ОШИБКА]"CL_RESET": Невозможно открыть '"CL_WHITE"%s"CL_RESET"', доступ запрещён.\n", DEBUGLOGPATH);
			FFLUSH(STDERR);
		} else {
			fprintf(fp,"%s ", prefix);
			va_copy(apcopy, ap);
		...
		FPRINTF(STDERR, CL_RED"[ОШИБКА]"CL_RESET": DEBUGLOGPATH не определён!\n");
		FFLUSH(STDERR);
	}
#endif
