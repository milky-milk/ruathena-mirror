...
//define GET_VALUE
	...
	if (type > 0 && type < MAX_PACKET_DB) {
		...
		if(len)
		{// show packet length
			sprintf(atcmd_output, msg_txt(904), type, packet_db[1][type].len); // Packet 0x%x length: %d
			clif_displaymessage(fd, atcmd_output);
			return 0;
		}
...
ACMD_FUNC(clone)
{
	...
	if (!message || !*message) {
		clif_displaymessage(sd->fd,msg_txt(134)); // You must enter a player name or ID.
		return 0;
	}
